...
internal sealed class GroupJoinApplicationApprovedEmailHandler(
    IApplicationDbContext context,
    NotificationRecipientService recipientService,
    NotificationLinkBuilder linkBuilder,
    IEmailSender emailSender,
    ILogger<GroupJoinApplicationApprovedEmailHandler> logger) : IGroupJoinApplicationApprovedEmailHandler
{
    public async Task HandleAsync(
        GroupJoinApplicationApproved domainEvent,
        CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogWarning(
                "Skipping join application approved email for user {UserId}: no email",
                domainEvent.UserId);
            return;
        }
        ...
        var body = $"""
            Your application to join "{groupName}" was approved.
        ...
        await emailSender.SendAsync(
            new EmailMessage(
                [recipient],
                $"Application approved — {groupName}",
                body),
            cancellationToken);
    }
}
